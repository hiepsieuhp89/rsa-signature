...
function writeFile(expectedName, zip) {
  if (path.resolve) {
    var _writeFile = path.resolve(examplesDirectory, "..", expectedName);
...
function shouldBeSame(_ref) {
  ...
  if (!documentCache[expectedName]) {
    ...
    throw new Error("File ${expectedName} does not exist in examples directory");
  }
  ...
  try {
    uniq(Object.keys(zip.files).concat(Object.keys(expectedZip.files))).map(function (filePath) {
      ...
      expect(file).to.be.an("object", "The file ".concat(filePath, " doesn't exist on generated file"));
      ...
      var isImg = filePath.indexOf(".png") !== -1 || filePath.indexOf(".jpeg") !== -1;
